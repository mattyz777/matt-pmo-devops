...
CREATE INDEX idx_release_docs_is_deleted    ON release_docs (is_deleted);
...
COMMENT ON COLUMN release_docs.is_deleted IS '是否删除';
COMMENT ON COLUMN release_docs.created_at IS '创建时间';
COMMENT ON COLUMN release_docs.updated_at IS '更新时间';
COMMENT ON COLUMN release_docs.created_by IS '创建人ID';
COMMENT ON COLUMN release_docs.updated_by IS '更新人ID';
...
CREATE TABLE users (
    ...
    kind                INTEGER      NOT NULL,  -- 类型: normal=1,admin=2
    status              INTEGER      NOT NULL,  -- 状态: enabled=1,disabled=2
...
COMMENT ON TABLE users IS '发布文档表';
COMMENT ON COLUMN users.kind IS '类型: normal=1,admin=2';
COMMENT ON COLUMN users.status IS '状态: enabled=1,disabled=2';
